...
import os
import time
from utils.utils_old import calculate_openai_api_cost
...
def translate_to_chinese(session_id, text, messages_trace, cost_tracker):
    ...
    try:
        ...
        translation_prompt = langfuse.get_prompt(
            "translation_prompt", label="production"
        )
        ...
        translated_text = (
            response.choices[0].message.to_dict().get("content", "").strip()
        )
        print(f"Translated text: {translated_text}")

        openai_cost = calculate_openai_api_cost(
            response, translation_prompt.config["model"]
        )
