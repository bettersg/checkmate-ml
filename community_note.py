...
from utils.utils_old import (
    calculate_openai_api_cost,
    call_tool,
    remove_user_links_from_sources,
)
import os
...
search_google_tool_str = langfuse.get_prompt(
    "search_google_tool", label="production"
).prompt
get_website_screenshot_tool_str = langfuse.get_prompt(
    "get_website_screenshot_tool", label="production"
).prompt
submit_community_note_tool_str = langfuse.get_prompt(
    "submit_community_note_tool", label="production"
).prompt
...
async def generate_community_note(
    session_id,
    data_type: str = "text",
    ...
    try:
        while len(messages) < 20 and not completed:
            ...
            messages.append(response.choices[0].message)
            openai_cost = calculate_openai_api_cost(
                response, system_prompt.config["model"]
            )
            cost_tracker["total_cost"] += openai_cost
            cost_tracker["cost_trace"].append(
                {
            ...
            if tool_calls:
                ...
                for tool_call in tool_calls:
                    ...
                    if tool_name == "submit_community_note":
                        ...
                        # summarise the note
                        final_messages, final_note = await summary_note(
                            session_id, messages, cost_tracker
                        )

                        # translate the note to chinese
                        chinese_note, final_messages = translate_to_chinese(
                            session_id, final_note, final_messages, cost_tracker
                        )
                        ...
                        final_sources = remove_user_links_from_sources(
                            user_input, arguments.get("sources", [])
                        )
                        ...
                        final_note_items = CommunityNoteItem(
                            en=final_note,
                            cn=chinese_note,
                            links=final_sources,
                        ...
                        tool_call_promise = call_tool(
                            tool_dict, tool_name, arguments, tool_call_id, cost_tracker
                        )
                        tool_call_promises.append(tool_call_promise)
                tool_results = await asyncio.gather(*tool_call_promises)
                flattened_results = [
                    item
                    for sublist in tool_results
                    for item in (sublist if isinstance(sublist, list) else [sublist])
                ]

                # Separate user messages and tool messages
                user_messages = [
                    msg for msg in flattened_results if msg.get("role") != "tool"
                ]
                tool_messages = [
                    msg for msg in flattened_results if msg.get("role") == "tool"
                ]
        ...
        raise HTTPException(
            status_code=400,
            detail={"error": "Bad Request", "message": str(e), "code": "INVALID_INPUT"},
        )
